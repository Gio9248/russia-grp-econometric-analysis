# Анализ данных регионального развития России: влияние инвестиций на ВРП

## TLDR

- Вопрос: влияют ли инвестиции в основной капитал на ВРП регионов России, если учесть, что сами инвестиции могут быть следствием ВРП, а не только его причиной (эндогенность).
- Данные: 80 регионов РФ за 2024 год (Росстат), после чистки пропусков и выбросов.
- Наивная OLS-оценка эластичности инвестиций — 0.5241 (R² = 0.869), но тест на эндогенность (Ву-Хаусман через контрольную функцию и тест Вулдриджа, p ≈ 0.02) показывает, что эта оценка смещена.
- После перехода на 2SLS с инструментом (протяжённость дорог, инструмент релевантен — F = 11.75, порог Стока-Його пройден с запасом) эластичность выросла до 0.7571, значима на 1%: рост инвестиций на душу на 1% связан с ростом ВРП на душу примерно на 0.76%.
- Вывод устойчив к робастным стандартным ошибкам и разным тестам на гетероскедастичность.

Учебный эконометрический проект о том, насколько инвестиции в основной капитал объясняют разброс ВРП на душу населения между регионами России. На первый взгляд, вопрос закрыт ещё на первом семинаре по макре (Y = C + G + I + NX: инвестиции растут, растёт и ВРП). Но часть зарубежной литературы находит эффект инвестиций в инфраструктуру незначимым или даже отрицательным, и одна из вероятных причин заключается в эндогенности: инвестиции далеко не всегда экзогенный фактор, а зачастую сами являются следствием того же ВРП, который должны объяснять. Весь проект построен вокруг этой проблемы: сначала честная OLS-оценка, потом её проверка на эндогенность и переход на 2SLS с инструментальной переменной.
...
Сырые данные из Росстата выгружены в формате, удобном для чтения: пробел вместо разделителя тысяч (1 015 794), запятая вместо точки, знак процента прямо внутри ячейки. Всё это прогоняется через `str.replace` (убираются пробелы, запятые меняются на точки, `%` вырезается), а затем через `pd.to_numeric(errors='coerce')`, благодаря чему всё, что не преобразовалось в число, сразу становится видимым NaN.

После этого остаётся 3 пропуска, и все по протяжённости дорог: у Москвы, Санкт-Петербурга и Севастополя такой статистики нет ни у Росстата, ни в других открытых источниках. Удобное совпадение: у городов федерального значения структура ВРП и так принципиально другая (в основном сфера услуг, а не добыча и обработка), так что их всё равно пришлось бы чистить как выбросы чуть позже. 85 → 82 наблюдения.

Дальше построены боксплоты. Ненецкий и Ямало-Ненецкий АО вылетают за пределы графика по ВРП на душу населения (14.25 и 12.2 млн руб. против медианы в районе 800 тыс.) — маленькое население, да и огромная добывающая промышленность. Это реальные регионы, не ошибка в данных, но для линейной модели такие два наблюдения рискуют утянуть коэффициенты за собой. Убраны как выбросы. В итоге осталось 80 наблюдений.

![Боксплоты переменных после очистки данных](assets/boxplots_clean.png)

### Логарифмирование

ВРП на душу, численность рабочей силы, инвестиции на душу и протяжённость дорог сильно скошены вправо — медиана заметно ниже среднего, типичная картина для показателей вида «чем крупнее регион, тем непропорционально больше значение». Прологарифмированы через `log1p` (хотя нулевых значений в выборке в итоге не оказалось, это скорее стандартная подстраховка). После логарифмирования распределения куда ближе к нормальному, и заодно коэффициенты модели превращаются в эластичности, что удобно для интерпретации «на сколько % вырастет ВРП при росте инвестиций на 1%». Доли (городское население, добыча и обработка) оставлены как есть — они и так ограничены диапазоном 0–100.
