...
Сырые данные из Росстата выгружены в формате для человеческого глаза, а не для pandas: пробел вместо разделителя тысяч (1 015 794), запятая вместо точки, знак процента прямо внутри ячейки. Всё это прогоняется через `str.replace` (убираются пробелы, запятые меняются на точки, `%` вырезается), а затем через `pd.to_numeric(errors='coerce')` — так всё, что не преобразовалось в число, сразу становится видимым NaN.
...
Дальше — боксплоты. Ненецкий и Ямало-Ненецкий АО вылетают за пределы графика по ВРП на душу населения (14.25 и 12.2 млн руб. против медианы в районе 800 тыс.) — маленькое население плюс огромная добывающая промышленность. Это реальные регионы, не ошибка в данных, но для линейной модели такие два наблюдения — готовый рычаг, который утянет коэффициенты за собой. Убраны как выбросы. Итог — 80 наблюдений.
...
**Первый этап**: log(инвестиций) регрессируется на инструмент (log длины дорог) и все экзогенные контроли. Коэффициент при инструменте отрицательный и значим на 1% (-0.4539, t = -3.43) — сам знак тут не так важен, важно, что связь сильная и статистически надёжная.
...
**Гетероскедастичность.** Тест Бройша-Пагана (ловит линейную форму гетероскедастичности) проблем не находит (p = 0.347), тест Уайта (более общая форма, с перекрёстными членами) — находит (p = 0.0023). Расхождение не редкость: Бройш-Паган слеп к нелинейным и перекрёстным эффектам, которые как раз ловит Уайт. Раз хотя бы один тест сигналит проблему, безопаснее её учесть.
...
Кстати, тот факт, что после исправления на эндогенность эффект стал сильнее, а не слабее наивной OLS-оценки — довольно частый паттерн в литературе по инвестициям, и он скорее говорит в пользу того, что связь реальная, а не просто следствие того, что более богатые регионы могут себе позволить больше вкладывать.
